...
const APP_ID: &str = "com.pokedia.app";
...
const SEARCH_RESULTS_PER_SECTION: usize = 64;
...
const SEARCH_RESULTS_STACK_NAME: &str = "search-results";
static WORKSPACE_SAVE_SEQUENCE: AtomicU64 = AtomicU64::new(0);
...
.chart-axis.chart-dragging {
  background: rgba(255, 111, 144, .16);
  box-shadow:
    inset 0 0 0 1px rgba(255, 111, 144, .56),
    0 0 16px rgba(255, 111, 144, .16);
  color: #ffffff;
  opacity: .72;
}

.chart-axis.chart-drop-target {
  background: rgba(102, 227, 223, .10);
  box-shadow:
    inset 0 0 0 1px rgba(102, 227, 223, .42),
    0 0 16px rgba(102, 227, 223, .12);
  color: #ffffff;
}

.chart-axis.chart-drop-column-before {
  box-shadow:
    inset 3px 0 0 rgba(102, 227, 223, .95),
    inset 0 0 0 1px rgba(102, 227, 223, .42),
    0 0 16px rgba(102, 227, 223, .12);
}

.chart-axis.chart-drop-column-after {
  box-shadow:
    inset -3px 0 0 rgba(102, 227, 223, .95),
    inset 0 0 0 1px rgba(102, 227, 223, .42),
    0 0 16px rgba(102, 227, 223, .12);
...
.chart-axis.chart-drop-row-before {
  box-shadow:
    inset 0 3px 0 rgba(102, 227, 223, .95),
    inset 0 0 0 1px rgba(102, 227, 223, .42),
    0 0 16px rgba(102, 227, 223, .12);
}

.chart-axis.chart-drop-row-after {
  box-shadow:
    inset 0 -3px 0 rgba(102, 227, 223, .95),
    inset 0 0 0 1px rgba(102, 227, 223, .42),
    0 0 16px rgba(102, 227, 223, .12);
...
impl FilterDropdown {
    ...
    fn set_selected(&self, selected: u32) {
        let selected = self.normalized_selected(selected);
    ...
    fn set_selected_silent(&self, selected: u32) {
        let selected = self.normalized_selected(selected);
        self.selected.set(selected);
        self.refresh();
    }

    fn normalized_selected(&self, selected: u32) -> u32 {
...
#[derive(Clone)]
struct SearchResultsWidgets {
    page: gtk::Box,
    sections: gtk::Box,
    empty: gtk::Label,
    pokemon_section: SearchResultSection,
    move_section: SearchResultSection,
    ability_section: SearchResultSection,
    item_section: SearchResultSection,
    nature_section: SearchResultSection,
    pokemon_rows: gtk::Box,
    move_rows: gtk::Box,
    ability_rows: gtk::Box,
    item_rows: gtk::Box,
    nature_rows: gtk::Box,
}
...
struct AppWidgets {
    ...
    search_revision: Rc<Cell<u64>>,
    suppress_model_refresh: Rc<Cell<bool>>,
    ...
    filtered_natures: Rc<RefCell<Vec<NatureSummary>>>,
    ...
    search_results: SearchResultsWidgets,
    search_mode_active: Rc<Cell<bool>>,
...
struct DetailLoader {
    ...
    sender: mpsc::Sender<DetailLoadResult>,
}

struct PokemonDetailPayload {
    detail: Option<PokemonDetail>,
    abilities: Vec<PokemonAbility>,
    moves: Vec<PokemonMoveEntry>,
    evolution: Option<EvolutionNode>,
    alternate_forms: Vec<PokemonSummary>,
...
enum DetailLoadResult {
    Pokemon {
        revision: u64,
        payload: Result<PokemonDetailPayload, String>,
    },
    Move {
        revision: u64,
        payload: Result<(Option<MoveDetail>, Vec<MovePokemonEntry>), String>,
    },
    Ability {
        revision: u64,
        payload: Result<(Option<AbilityDetail>, Vec<AbilityPokemonEntry>), String>,
    },
    Item {
        revision: u64,
        payload: Result<ItemDetailPayload, String>,
    },
}

impl DetailLoader {
    fn new() -> (Self, mpsc::Receiver<DetailLoadResult>) {
    ...
    fn is_current(&self, revision: u64) -> bool {
        self.revision.get() == revision
    }
}

fn start_detail_result_pump(widgets: &AppWidgets, receiver: mpsc::Receiver<DetailLoadResult>) {
    let widgets = widgets.clone();
    glib::timeout_add_local(Duration::from_millis(16), move || {
        while let Ok(result) = receiver.try_recv() {
            apply_detail_load_result(&widgets, result);
        }
...
fn apply_detail_load_result(widgets: &AppWidgets, result: DetailLoadResult) {
    match result {
        DetailLoadResult::Pokemon { revision, payload } => {
            if !widgets.detail_loader.is_current(revision) {
                return;
            }
            let payload = match payload {
                Ok(payload) => payload,
                Err(error) => {
                    eprintln!("Failed to load Pokemon detail: {error}");
                    return;
                }
            };
            let Some(pokemon) = payload.detail else {
                return;
            };
            update_detail(
                &widgets.detail,
                ...
                &payload.abilities,
                &payload.moves,
                payload.evolution.as_ref(),
                &payload.alternate_forms,
                &payload.locations,
                payload.selected_game.as_ref(),
        ...
        DetailLoadResult::Move { revision, payload } => {
            if !widgets.detail_loader.is_current(revision) {
                return;
            }
            match payload {
                Ok((Some(move_), pokemon)) => {
                    update_move_detail(
                        &widgets.move_detail,
                        &move_,
                        &pokemon,
                ...
                Ok((None, _)) => {}
                Err(error) => eprintln!("Failed to load move detail: {error}"),
            }
        }
        DetailLoadResult::Ability { revision, payload } => {
            if !widgets.detail_loader.is_current(revision) {
                return;
            }
            match payload {
                Ok((Some(ability), pokemon)) => {
                ...
                Ok((None, _)) => {}
                Err(error) => eprintln!("Failed to load ability detail: {error}"),
            }
        }
        DetailLoadResult::Item { revision, payload } => {
            if !widgets.detail_loader.is_current(revision) {
                return;
            }
            match payload {
                Ok(payload) => {
                    let Some(item) = payload.detail else {
                        return;
                    };
                    update_item_detail(
                        &widgets.item_detail,
                ...
                Err(error) => eprintln!("Failed to load item detail: {error}"),
            }
        }
    }
}
...
fn build_ui(app: &adw::Application) {
    ...
    let filtered_natures = Rc::new(RefCell::new(Vec::new()));
    let (sprite_loader, sprite_receiver) = SpriteLoader::new();
    let (detail_loader, detail_receiver) = DetailLoader::new();
    ...
    let search_results = build_search_results_page();
    ...
    stack.add_named(&search_results.page, Some(SEARCH_RESULTS_STACK_NAME));
    ...
    let widgets = AppWidgets {
        ...
        toast_overlay,
        current_page,
        search_revision: Rc::new(Cell::new(0)),
        suppress_model_refresh: Rc::new(Cell::new(false)),
        pokemon_model,
        move_model,
        item_model,
        nature_model,
        ...
        compare_badge,
        search_results,
        search_mode_active: Rc::new(Cell::new(false)),
        detail,
        move_detail,
        ability_detail,
        item_detail,
    ...
    start_detail_result_pump(&widgets, detail_receiver);
    ...
    connect_search(&widgets, pool.clone(), runtime.clone(), data.clone());
    ...
    connect_search_results_activation(&widgets, pool.clone(), runtime.clone(), data.clone());
    ...
    connect_window_search_typing(&window, &widgets);
...
fn app_icon_path() -> Option<PathBuf> {
    let local_data_home = std::env::var_os("XDG_DATA_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".local/share")));

    let mut candidates = Vec::new();
    if let Some(local_data_home) = local_data_home {
        candidates.push(
            local_data_home
                .join("icons/hicolor/32x32/apps")
    ...
    candidates.push(Path::new(env!("CARGO_MANIFEST_DIR")).join("icons/32x32.png"));
...
fn persist_workspace_store(store: &WorkspaceStore) {
    ...
    let json = match serde_json::to_string_pretty(store) {
        Ok(json) => json,
        Err(error) => {
            eprintln!("Failed to serialize workspaces: {error}");
            return;
        }
    };
    let sequence = WORKSPACE_SAVE_SEQUENCE.fetch_add(1, AtomicOrdering::Relaxed) + 1;
    ...
    thread::spawn(move || {
        ...
        if WORKSPACE_SAVE_SEQUENCE.load(AtomicOrdering::Acquire) != sequence {
...
fn apply_workspace_snapshot(
    ...
    widgets.suppress_model_refresh.set(true);
    ...
    widgets.suppress_model_refresh.set(false);
...
fn set_dropdown_selected(dropdown: &FilterDropdown, selected: u32) {
    ...
    dropdown.set_selected_silent(selected);
}

fn show_workspace_home_state(widgets: &AppWidgets, data: &LoadedData, page: Page) {
    widgets.detail_loader.invalidate();
    widgets.search_mode_active.set(false);
...
fn build_search_results_page() -> SearchResultsWidgets {
    let page = page_box();
    ...
    let empty = gtk::Label::new(Some(
        "No results in Encyclopedia categories. Try a different query.",
    ));
    ...
    let (pokemon_panel, pokemon_rows) = search_results_table_panel(build_pokemon_header());
    let pokemon_section = search_results_section(Page::Pokedex, "Pokedex", &pokemon_panel);

    let (move_panel, move_rows) = search_results_table_panel(simple_header(&[
        ("Type", 92, false),
        ("Name", 150, true),
        ("Class", 92, false),
        ("Power", 54, false),
    ...
    let move_section = search_results_section(Page::Moves, "Moves", &move_panel);

    let (ability_panel, ability_rows) = search_results_table_panel(simple_header(&[
        ("Name", 170, false),
        ("Gen", 52, false),
        ("Effect", 240, true),
    ]));
    let ability_section = search_results_section(Page::Abilities, "Abilities", &ability_panel);

    let (item_panel, item_rows) = search_results_table_panel(simple_header(&[
        ("Name", 140, false),
        ("Category", 110, false),
        ("Effect", 190, true),
    ]));
    let item_section = search_results_section(Page::Items, "Items", &item_panel);

    let (nature_panel, nature_rows) = search_results_table_panel(simple_header(&[
        ("Nature", 96, false),
        ("Increased", 86, false),
        ("Decreased", 86, false),
        ("Likes", 58, false),
    ...
    let nature_section = search_results_section(Page::Natures, "Natures", &nature_panel);
...
fn search_results_table_panel(header: gtk::Box) -> (gtk::Box, gtk::Box) {
    let panel = gtk::Box::new(gtk::Orientation::Vertical, 0);
    panel.add_css_class("section-card");
...
fn build_types_page() -> gtk::Box {
    ...
    let (chart_grid, reset_order_button, drag_preview_label) = build_type_chart_grid();
    let chart_toolbar = gtk::Box::new(gtk::Orientation::Horizontal, 8);
    chart_toolbar.set_halign(gtk::Align::End);
    chart_toolbar.append(&drag_preview_label);
    chart_toolbar.append(&reset_order_button);
    chart_section.append(&chart_toolbar);
...
struct TypeChartDragSession {
    ...
    target_placement: Option<DropPlacement>,
}
...
fn build_type_chart_grid() -> (gtk::Grid, gtk::Button, gtk::Label) {
    ...
    let row_order: Rc<RefCell<Vec<String>>> = Rc::new(RefCell::new(default_type_order()));
    let column_order: Rc<RefCell<Vec<String>>> = Rc::new(RefCell::new(default_type_order()));
    let suppress_chart_click = Rc::new(Cell::new(false));
    let drag_session: Rc<RefCell<Option<TypeChartDragSession>>> = Rc::new(RefCell::new(None));
    let reset_order_button = gtk::Button::with_label("Reset order");
    reset_order_button.add_css_class("flat");
    reset_order_button.add_css_class("chart-reset-button");
    reset_order_button.set_tooltip_text(Some("Reset type chart order"));
    reset_order_button.set_sensitive(false);
    let drag_preview_label = gtk::Label::new(None);
    drag_preview_label.add_css_class("chart-drag-preview");
    drag_preview_label.set_visible(false);
    drag_preview_label.set_xalign(1.0);
    ...
    for header in row_headers.iter() {
        ...
        let suppress_chart_click_for_click = suppress_chart_click.clone();
        let type_key = header.type_key.clone();
        let click = gtk::GestureClick::new();
        click.connect_released(move |gesture, _, _, _| {
            if suppress_chart_click_for_click.get() {
                return;
            }
            if gesture
                .current_event_state()
    ...
    for header in row_headers.iter() {
        connect_type_axis_drag(
            &grid,
            header,
            ChartAxis::Attack,
            row_headers.clone(),
            column_headers.clone(),
            cells.clone(),
            row_order.clone(),
            column_order.clone(),
            drag_session.clone(),
            suppress_chart_click.clone(),
            reset_order_button.clone(),
            drag_preview_label.clone(),
        );
    }
    ...
    {
        let grid_for_reset = grid.clone();
        let row_headers_for_reset = row_headers.clone();
        let column_headers_for_reset = column_headers.clone();
        let cells_for_reset = cells.clone();
        let row_order_for_reset = row_order.clone();
        let column_order_for_reset = column_order.clone();
        let reset_order_button_for_reset = reset_order_button.clone();
        reset_order_button.connect_clicked(move |_| {
            *row_order_for_reset.borrow_mut() = default_type_order();
            *column_order_for_reset.borrow_mut() = default_type_order();
            apply_type_chart_order(
                &grid_for_reset,
                row_headers_for_reset.as_slice(),
                column_headers_for_reset.as_slice(),
                cells_for_reset.as_slice(),
                &row_order_for_reset.borrow(),
                &column_order_for_reset.borrow(),
            );
            set_reset_order_button_state(
                &reset_order_button_for_reset,
                &row_order_for_reset.borrow(),
                &column_order_for_reset.borrow(),
            );
        });
    ...
    (grid, reset_order_button, drag_preview_label)
}

fn default_type_order() -> Vec<String> {
    ALL_TYPES
        .iter()
        .map(|type_key| (*type_key).to_owned())
        .collect()
}

fn is_default_type_order(order: &[String]) -> bool {
    order.len() == ALL_TYPES.len()
        && order
            .iter()
            .zip(ALL_TYPES.iter())
            .all(|(current, default)| current == default)
}

fn set_reset_order_button_state(
    reset_order_button: &gtk::Button,
    row_order: &[String],
    column_order: &[String],
) {
    reset_order_button
        .set_sensitive(!is_default_type_order(row_order) || !is_default_type_order(column_order));
}

fn reorder_type_keys(
    order: &mut Vec<String>,
    dragged_type: &str,
    target_type: &str,
    placement: DropPlacement,
) -> bool {
    if dragged_type == target_type {
        return false;
    }

    let original = order.clone();
    let Some(dragged_index) = order.iter().position(|type_key| type_key == dragged_type) else {
        return false;
    };

    let dragged_value = order.remove(dragged_index);
    let Some(target_index) = order.iter().position(|type_key| type_key == target_type) else {
        *order = original;
        return false;
    };
    ...
    order.insert(insert_index, dragged_value);

    *order != original
}

fn apply_type_chart_order(
    grid: &gtk::Grid,
    ...
    for header in column_headers {
        if let Some(col) = column_order
            .iter()
            .position(|type_key| type_key == &header.type_key)
        {
            grid.remove(&header.label);
            grid.attach(&header.label, (col + 1) as i32, 0, 1, 1);
        }
    }

    for header in row_headers {
        if let Some(row) = row_order
            .iter()
            .position(|type_key| type_key == &header.type_key)
        {
            grid.remove(&header.label);
            grid.attach(&header.label, 0, (row + 1) as i32, 1, 1);
        }
    }

    for cell in cells {
        let Some(row) = row_order
            .iter()
            .position(|type_key| type_key == &cell.attacking)
        else {
            continue;
        };
        let Some(col) = column_order
            .iter()
            .position(|type_key| type_key == &cell.defending)
        else {
            continue;
        };

        grid.remove(&cell.label);
        grid.attach(&cell.label, (col + 1) as i32, (row + 1) as i32, 1, 1);
    }
}

fn type_chart_drag_target(
    grid: &gtk::Grid,
    headers: &[TypeChartHeader],
    axis: ChartAxis,
    dragged_type: &str,
    pointer_x: f64,
    pointer_y: f64,
) -> Option<(String, DropPlacement)> {
    let pointer_position = match axis {
        ChartAxis::Attack => pointer_y,
        ChartAxis::Defense => pointer_x,
    };
    let mut closest: Option<(String, DropPlacement, f64)> = None;

    for header in headers {
        if header.type_key == dragged_type {
            continue;
        }

        let Some(bounds) = header.label.compute_bounds(grid) else {
            continue;
        };
        let start = match axis {
            ChartAxis::Attack => bounds.y() as f64,
            ChartAxis::Defense => bounds.x() as f64,
        };
        let size = match axis {
            ChartAxis::Attack => bounds.height() as f64,
            ChartAxis::Defense => bounds.width() as f64,
        };
        let center = start + size / 2.0;
        let distance = (pointer_position - center).abs();
        let placement = if pointer_position > center {
            DropPlacement::After
        } else {
            DropPlacement::Before
        };

        if closest.as_ref().map_or(true, |(_, _, closest_distance)| {
            distance < *closest_distance
        }) {
            closest = Some((header.type_key.clone(), placement, distance));
        }
    }

    closest.map(|(type_key, placement, _)| (type_key, placement))
}

fn update_type_chart_drag_styles(
    row_headers: &[TypeChartHeader],
    column_headers: &[TypeChartHeader],
    axis: Option<ChartAxis>,
    dragged_type: Option<&str>,
    target_type: Option<&str>,
    target_placement: Option<DropPlacement>,
) {
    for header in row_headers {
        let dragging = axis == Some(ChartAxis::Attack)
            && dragged_type.is_some_and(|type_key| type_key == header.type_key);
        let target = axis == Some(ChartAxis::Attack)
            && target_type.is_some_and(|type_key| type_key == header.type_key)
            && !dragging;
        set_css_class(&header.label, "chart-dragging", dragging);
        set_css_class(&header.label, "chart-drop-target", target);
        set_css_class(
            &header.label,
            "chart-drop-row-before",
            target && target_placement == Some(DropPlacement::Before),
        );
        set_css_class(
            &header.label,
            "chart-drop-row-after",
            target && target_placement == Some(DropPlacement::After),
        );
        set_css_class(&header.label, "chart-drop-column-before", false);
        set_css_class(&header.label, "chart-drop-column-after", false);
    }

    for header in column_headers {
        let dragging = axis == Some(ChartAxis::Defense)
            && dragged_type.is_some_and(|type_key| type_key == header.type_key);
        let target = axis == Some(ChartAxis::Defense)
            && target_type.is_some_and(|type_key| type_key == header.type_key)
            && !dragging;
        set_css_class(&header.label, "chart-dragging", dragging);
        set_css_class(&header.label, "chart-drop-target", target);
        set_css_class(&header.label, "chart-drop-row-before", false);
        set_css_class(&header.label, "chart-drop-row-after", false);
        set_css_class(
            &header.label,
            "chart-drop-column-before",
            target && target_placement == Some(DropPlacement::Before),
        );
        set_css_class(
            &header.label,
            "chart-drop-column-after",
            target && target_placement == Some(DropPlacement::After),
        );
    }
}

fn set_type_chart_drag_preview(
    drag_preview_label: &gtk::Label,
    axis: ChartAxis,
    dragged_type: &str,
    target_type: Option<&str>,
    target_placement: Option<DropPlacement>,
    ...
    let axis_label = match axis {
        ChartAxis::Attack => "row",
        ChartAxis::Defense => "column",
    };
    let text = match (target_type, target_placement) {
        (Some(target_type), Some(placement)) if target_type != dragged_type => {
            let placement_label = match placement {
                DropPlacement::Before => "before",
                DropPlacement::After => "after",
            };
            format!(
                "Move {} {axis_label} {placement_label} {}",
                native::titleize_key(dragged_type),
                native::titleize_key(target_type),
            )
        }
        _ => format!(
            "Dragging {} {axis_label}",
    ...
    drag_preview_label.set_text(&text);
    drag_preview_label.set_visible(true);
}

fn clear_type_chart_drag_preview(drag_preview_label: &gtk::Label) {
    drag_preview_label.set_text("");
    drag_preview_label.set_visible(false);
}

fn release_type_chart_click_suppression(suppress_chart_click: Rc<Cell<bool>>) {
    suppress_chart_click.set(true);
    glib::timeout_add_local_once(Duration::from_millis(180), move || {
        suppress_chart_click.set(false);
    });
}

#[allow(clippy::too_many_arguments)]
fn connect_type_axis_drag(
    grid: &gtk::Grid,
    header: &TypeChartHeader,
    axis: ChartAxis,
    row_headers: Rc<Vec<TypeChartHeader>>,
    column_headers: Rc<Vec<TypeChartHeader>>,
    cells: Rc<Vec<TypeChartCell>>,
    row_order: Rc<RefCell<Vec<String>>>,
    column_order: Rc<RefCell<Vec<String>>>,
    drag_session: Rc<RefCell<Option<TypeChartDragSession>>>,
    suppress_chart_click: Rc<Cell<bool>>,
    reset_order_button: gtk::Button,
    drag_preview_label: gtk::Label,
) {
    ...
    let grid_for_begin = grid.clone();
    let label_for_begin = header.label.clone();
    let type_key_for_begin = header.type_key.clone();
    let row_headers_for_begin = row_headers.clone();
    let column_headers_for_begin = column_headers.clone();
    let drag_session_for_begin = drag_session.clone();
    let drag_preview_label_for_begin = drag_preview_label.clone();
    gesture.connect_drag_begin(move |_, start_x, start_y| {
        let Some(bounds) = label_for_begin.compute_bounds(&grid_for_begin) else {
            return;
        };

        *drag_session_for_begin.borrow_mut() = Some(TypeChartDragSession {
            axis,
            type_key: type_key_for_begin.clone(),
            origin_x: bounds.x() as f64,
            origin_y: bounds.y() as f64,
            start_x,
            start_y,
            active: false,
            target_type: None,
        ...
        update_type_chart_drag_styles(
            row_headers_for_begin.as_slice(),
            column_headers_for_begin.as_slice(),
            Some(axis),
            Some(type_key_for_begin.as_str()),
            None,
            None,
        );
        set_type_chart_drag_preview(
            &drag_preview_label_for_begin,
            axis,
            type_key_for_begin.as_str(),
            None,
            None,
        );
    });

    let grid_for_update = grid.clone();
    let row_headers_for_update = row_headers.clone();
    let column_headers_for_update = column_headers.clone();
    let drag_session_for_update = drag_session.clone();
    let suppress_chart_click_for_update = suppress_chart_click.clone();
    let drag_preview_label_for_update = drag_preview_label.clone();
    gesture.connect_drag_update(move |_, offset_x, offset_y| {
        let (drag_axis, dragged_type, pointer_x, pointer_y) = {
            let mut session = drag_session_for_update.borrow_mut();
            ...
            if !session.active && distance < 5.0 {
                return;
            }

            if !session.active {
                session.active = true;
                suppress_chart_click_for_update.set(true);
            }

            (
                session.axis,
                session.type_key.clone(),
                session.origin_x + session.start_x + offset_x,
                session.origin_y + session.start_y + offset_y,
            )
        };

        let headers = match drag_axis {
            ChartAxis::Attack => row_headers_for_update.as_slice(),
            ChartAxis::Defense => column_headers_for_update.as_slice(),
        };
        let Some((target_type, placement)) = type_chart_drag_target(
            &grid_for_update,
            headers,
            drag_axis,
            &dragged_type,
        ...
        if let Some(session) = drag_session_for_update.borrow_mut().as_mut() {
            session.target_type = Some(target_type.clone());
            session.target_placement = Some(placement);
        }

        update_type_chart_drag_styles(
            row_headers_for_update.as_slice(),
            column_headers_for_update.as_slice(),
            Some(drag_axis),
            Some(dragged_type.as_str()),
            Some(target_type.as_str()),
            Some(placement),
        );
        set_type_chart_drag_preview(
            &drag_preview_label_for_update,
            drag_axis,
            dragged_type.as_str(),
            Some(target_type.as_str()),
            Some(placement),
    ...
    let row_headers_for_end = row_headers.clone();
    let column_headers_for_end = column_headers.clone();
    let cells_for_end = cells.clone();
    let row_order_for_end = row_order.clone();
    let column_order_for_end = column_order.clone();
    let grid_for_end = grid.clone();
    let drag_session_for_end = drag_session.clone();
    let suppress_chart_click_for_end = suppress_chart_click.clone();
    let reset_order_button_for_end = reset_order_button.clone();
    let drag_preview_label_for_end = drag_preview_label.clone();
    gesture.connect_drag_end(move |_, _, _| {
        let ended_session = drag_session_for_end.borrow_mut().take();
        let was_active = ended_session.as_ref().is_some_and(|session| session.active);

        if let Some(session) = ended_session.as_ref() {
            if session.active {
                if let (Some(target_type), Some(target_placement)) =
                    (session.target_type.as_deref(), session.target_placement)
                {
                    let changed = match session.axis {
                        ChartAxis::Attack => reorder_type_keys(
                            &mut row_order_for_end.borrow_mut(),
                            &session.type_key,
                            target_type,
                            target_placement,
                        ),
                        ChartAxis::Defense => reorder_type_keys(
                            &mut column_order_for_end.borrow_mut(),
                            &session.type_key,
                            target_type,
                            target_placement,
                        ),
                    ...
                    if changed {
                        apply_type_chart_order(
                            &grid_for_end,
                            row_headers_for_end.as_slice(),
                            column_headers_for_end.as_slice(),
                            cells_for_end.as_slice(),
                            row_order_for_end.borrow().as_slice(),
                            column_order_for_end.borrow().as_slice(),
                        );
                        set_reset_order_button_state(
                            &reset_order_button_for_end,
                            row_order_for_end.borrow().as_slice(),
                            column_order_for_end.borrow().as_slice(),
                        );
                    }
        ...
        clear_type_chart_drag_preview(&drag_preview_label_for_end);

        if was_active {
            release_type_chart_click_suppression(suppress_chart_click_for_end.clone());
        }
    });

    header.label.add_controller(gesture);
}
...
fn filter_pokemon_summaries(
    widgets: &AppWidgets,
    all_pokemon: &[PokemonSummary],
    query: &str,
) -> Vec<PokemonSummary> {
    ...
    let search_tokens = folded_search_tokens(query);

    let mut rows = all_pokemon
        .iter()
        .filter(|pokemon| pokemon_matches_search_tokens(pokemon, &search_tokens))
        .filter(|pokemon| pokemon_matches_type_filters(pokemon, type_filter, second_type_filter))
        .filter(|pokemon| {
            generation_range.map_or(true, |(min, max)| {
...
fn filter_move_summaries(
    ...
    let search_tokens = lowercase_search_tokens(query);
    ...
    moves
        ...
        .filter(|move_| {
            type_filter.map_or(true, |type_key| move_.type_key.as_deref() == Some(type_key))
        })
        .filter(|move_| {
            class_filter.map_or(true, |class| move_.damage_class.as_deref() == Some(class))
        })
        .filter(|move_| min_power.map_or(true, |min| move_.power.is_some_and(|power| power >= min)))
        .filter(|move_| max_power.map_or(true, |max| move_.power.is_some_and(|power| power <= max)))
        .cloned()
        .collect()
}

fn filter_ability_summaries(
    widgets: &AppWidgets,
    abilities: &[AbilitySummary],
    query: &str,
) -> Vec<AbilitySummary> {
    ...
    abilities
        ...
        .filter(|ability| {
            generation_filter.map_or(true, |generation| ability.generation == Some(generation))
        })
        .cloned()
        .collect()
}

fn filter_item_summaries(
    widgets: &AppWidgets,
    items: &[ItemSummary],
    query: &str,
) -> Vec<ItemSummary> {
    ...
    items
        ...
        .filter(|item| {
            category_filter.map_or(true, |category| item.category.as_deref() == Some(category))
        })
        .cloned()
        .collect()
}

fn filter_nature_summaries(
    widgets: &AppWidgets,
...
fn refresh_search_data(widgets: &AppWidgets, data: &LoadedData, query: &str) {
    let pokemon = filter_pokemon_summaries(widgets, &data.pokemon, query);
    widgets
        .pokemon_count
        .set_text(&format!("{} Pokemon", pokemon.len()));
    widgets.filtered_pokemon.replace(pokemon);

    let moves = filter_move_summaries(widgets, &data.moves, query);
    widgets
        .move_count
        .set_text(&format!("{} moves", moves.len()));
    widgets.filtered_moves.replace(moves);

    let abilities = filter_ability_summaries(widgets, &data.abilities, query);
    widgets
        .ability_count
        .set_text(&format!("{} abilities", abilities.len()));
    widgets.filtered_abilities.replace(abilities);

    let items = filter_item_summaries(widgets, &data.items, query);
    widgets
        .item_count
        .set_text(&format!("{} items", items.len()));
    widgets.filtered_items.replace(items);

    let natures = filter_nature_summaries(widgets, &data.natures, query);
    widgets
        .nature_count
        .set_text(&format!("{} natures", natures.len()));
    widgets.filtered_natures.replace(natures);
}
...
fn is_search_query_active(query: &str) -> bool {
    !query.trim().is_empty()
}

fn encyclopedia_search_order(page: Page) -> Vec<Page> {
    let mut order = vec![
        Page::Pokedex,
        Page::Moves,
        Page::Abilities,
    ...
    if matches!(
        page,
        Page::Pokedex | Page::Moves | Page::Abilities | Page::Items | Page::Natures
    ) {
        order.retain(|entry| *entry != page);
        order.insert(0, page);
    }
...
fn encyclopedia_category_label(page: Page) -> &'static str {
    match page {
        Page::Pokedex => "Pokedex",
        Page::Moves => "Moves",
        Page::Abilities => "Abilities",
        Page::Items => "Items",
        Page::Natures => "Natures",
        Page::Types | Page::Compare | Page::Settings => "",
    }
}

fn search_result_section(results: &SearchResultsWidgets, page: Page) -> &SearchResultSection {
    match page {
        Page::Pokedex => &results.pokemon_section,
        Page::Moves => &results.move_section,
        Page::Abilities => &results.ability_section,
        Page::Items => &results.item_section,
        Page::Natures => &results.nature_section,
        Page::Types | Page::Compare | Page::Settings => &results.pokemon_section,
    }
}

fn search_result_count(widgets: &AppWidgets, page: Page) -> usize {
    match page {
        Page::Pokedex => widgets.filtered_pokemon.borrow().len(),
        Page::Moves => widgets.filtered_moves.borrow().len(),
        Page::Abilities => widgets.filtered_abilities.borrow().len(),
        Page::Items => widgets.filtered_items.borrow().len(),
        Page::Natures => widgets.filtered_natures.borrow().len(),
        Page::Types | Page::Compare | Page::Settings => 0,
    }
}

fn refresh_search_results_page(widgets: &AppWidgets, _query: &str) {
    clear_box(&widgets.search_results.sections);
    render_search_result_rows(widgets);

    let order = encyclopedia_search_order(*widgets.current_page.borrow());
    let mut total = 0usize;
    for page in order {
        let count = search_result_count(widgets, page);
        if count == 0 {
            continue;
        }
        total += count;
        let section = search_result_section(&widgets.search_results, page);
        let category = encyclopedia_category_label(section.page);
        if count > SEARCH_RESULTS_PER_SECTION {
            section
                .title
                .set_text(&format!("{category} ({SEARCH_RESULTS_PER_SECTION}/{count})"));
        } else {
            section.title.set_text(&format!("{category} ({count})"));
        }
        widgets.search_results.sections.append(&section.container);
    }

    widgets.search_results.empty.set_visible(total == 0);
    widgets.search_results.sections.set_visible(total > 0);
}

fn render_search_result_rows(widgets: &AppWidgets) {
    clear_box(&widgets.search_results.pokemon_rows);
    for pokemon in widgets
        .filtered_pokemon
        .borrow()
        .iter()
        .take(SEARCH_RESULTS_PER_SECTION)
    {
        widgets
            .search_results
            .pokemon_rows
            .append(&search_pokemon_result_row(pokemon, &widgets.sprite_loader));
    }

    clear_box(&widgets.search_results.move_rows);
    for move_ in widgets
        .filtered_moves
        .borrow()
        .iter()
        .take(SEARCH_RESULTS_PER_SECTION)
    {
        widgets
            .search_results
            .move_rows
            .append(&search_move_result_row(move_));
    }

    clear_box(&widgets.search_results.ability_rows);
    for ability in widgets
        .filtered_abilities
        .borrow()
        .iter()
        .take(SEARCH_RESULTS_PER_SECTION)
    {
        widgets
            .search_results
            .ability_rows
            .append(&search_ability_result_row(ability));
    }

    clear_box(&widgets.search_results.item_rows);
    for item in widgets
        .filtered_items
        .borrow()
        .iter()
        .take(SEARCH_RESULTS_PER_SECTION)
    {
        widgets
            .search_results
            .item_rows
            .append(&search_item_result_row(item));
    }

    clear_box(&widgets.search_results.nature_rows);
    for nature in widgets
        .filtered_natures
        .borrow()
        .iter()
        .take(SEARCH_RESULTS_PER_SECTION)
    {
        widgets
            .search_results
            .nature_rows
            .append(&search_nature_result_row(nature));
    }
}

fn search_pokemon_result_row(pokemon: &PokemonSummary, sprite_loader: &SpriteLoader) -> gtk::Box {
    let row = gtk::Box::new(gtk::Orientation::Horizontal, 6);
    row.add_css_class("data-row");
    row.set_widget_name(&format!("pokemon:{}", pokemon.id));
    row.set_halign(gtk::Align::Fill);
    row.set_hexpand(true);

    let sprite = gtk::Image::from_icon_name("image-x-generic-symbolic");
    sprite.set_pixel_size(34);
    load_sprite(sprite_loader, &sprite, pokemon.sprite_url.as_deref(), 34);
    row.append(&sprite_frame(&sprite, 40, "sprite-frame"));

    let base_id = pokemon.species_id.unwrap_or(pokemon.id);
    row.append(&sized_label(&format!("{base_id:03}"), 30, false, "dex-id"));

    let name_box = gtk::Box::new(gtk::Orientation::Horizontal, 5);
    name_box.set_width_request(84);
    name_box.set_hexpand(true);
    let name = sized_label(&display_name(pokemon), 0, true, "row-title");
    let form = form_label(&pokemon.name_key).unwrap_or_default();
    let form = sized_label(&form, 0, false, "muted");
    form.set_visible(!form.text().is_empty());
    name_box.append(&name);
    name_box.append(&form);
    row.append(&name_box);
    ...
    for (key, class_name) in [
        ...
        row.append(&sized_label(
            &pokemon_stat_value(pokemon, key),
            34,
            false,
            class_name,
        ));
    }
    row.append(&sized_label("", 26, false, "muted"));

    row
}

fn search_move_result_row(move_: &MoveSummary) -> gtk::Box {
    let row = gtk::Box::new(gtk::Orientation::Horizontal, 8);
    row.add_css_class("data-row");
    row.set_widget_name(&format!("move:{}", move_.id));
    row.set_halign(gtk::Align::Fill);
    row.set_hexpand(true);
...
fn search_ability_result_row(ability: &AbilitySummary) -> gtk::Box {
    let row = gtk::Box::new(gtk::Orientation::Horizontal, 8);
    row.add_css_class("data-row");
    row.set_widget_name(&format!("ability:{}", ability.id));
    row.set_halign(gtk::Align::Fill);
    row.set_hexpand(true);
    row.append(&sized_label(
        &native::ability_summary_name(ability),
        170,
        false,
        "row-title",
    ));
    ...
    row.append(&sized_label(
        &ability
            ...
            .map(|text| inline_preview_text(text, 180))
            .unwrap_or_default(),
        240,
        true,
        "muted",
...
fn search_item_result_row(item: &ItemSummary) -> gtk::Box {
    let row = gtk::Box::new(gtk::Orientation::Horizontal, 8);
    row.add_css_class("data-row");
    row.set_widget_name(&format!("item:{}", item.id));
    row.set_halign(gtk::Align::Fill);
    row.set_hexpand(true);
    row.append(&sized_label(
    ...
    row.append(&sized_label(
        &item
            ...
            .map(|text| inline_preview_text(text, 220))
            .unwrap_or_default(),
        190,
        true,
        "muted",
...
fn search_nature_result_row(nature: &NatureSummary) -> gtk::Box {
    let row = gtk::Box::new(gtk::Orientation::Horizontal, 6);
    row.add_css_class("data-row");
    row.set_halign(gtk::Align::Fill);
    row.set_hexpand(true);
...
fn apply_search_mode(
    ...
    if is_search_query_active(query) {
        widgets.detail_loader.invalidate();
        refresh_search_results_page(widgets, query);
        widgets
            .stack
            .set_visible_child_name(SEARCH_RESULTS_STACK_NAME);
        widgets.search_mode_active.set(true);
        return;
    }

    if !widgets.search_mode_active.replace(false) {
        return;
    }

    if let Some(page) = widgets.tab_view.selected_page() {
        if page != widgets.home_tab {
            if let Some(target) = selected_tab_target(widgets, &page) {
                show_tab_target(widgets, pool, runtime, data, &target);
                return;
            }
        }
    ...
    widgets.detail_loader.invalidate();
    if *widgets.current_page.borrow() == Page::Compare {
        render_compare_page(
            &widgets.compare,
            data.pokemon.clone(),
            widgets.compare_ids.clone(),
...
fn refresh_pokemon_model(widgets: &AppWidgets, all_pokemon: &[PokemonSummary], query: &str) {
    let rows = filter_pokemon_summaries(widgets, all_pokemon, query);
...
fn refresh_move_model(widgets: &AppWidgets, moves: &[MoveSummary], query: &str) {
    let filtered = filter_move_summaries(widgets, moves, query);
    let rows = filtered.iter().map(move_row_string).collect::<Vec<_>>();
    replace_string_list(&widgets.move_model, &rows);
    widgets
        .move_count
        .set_text(&format!("{} moves", rows.len()));
    widgets.filtered_moves.replace(filtered);
}

fn refresh_ability_flow(widgets: &AppWidgets, abilities: &[AbilitySummary], query: &str) {
    clear_flow_box(&widgets.ability_flow);
    let filtered = filter_ability_summaries(widgets, abilities, query);
    ...
    widgets
        .ability_count
        .set_text(&format!("{} abilities", filtered.len()));
    widgets.filtered_abilities.replace(filtered);
}

fn refresh_item_model(widgets: &AppWidgets, items: &[ItemSummary], query: &str) {
    let filtered = filter_item_summaries(widgets, items, query);
    let rows = filtered.iter().map(item_row_string).collect::<Vec<_>>();
    replace_string_list(&widgets.item_model, &rows);
    widgets
        .item_count
        .set_text(&format!("{} items", rows.len()));
    widgets.filtered_items.replace(filtered);
}

fn refresh_nature_model(widgets: &AppWidgets, natures: &[NatureSummary], query: &str) {
    let filtered = filter_nature_summaries(widgets, natures, query);
    let rows = filtered.iter().map(nature_row_string).collect::<Vec<_>>();
    replace_string_list(&widgets.nature_model, &rows);
    widgets
        .nature_count
        .set_text(&format!("{} natures", rows.len()));
    widgets.filtered_natures.replace(filtered);
}
...
fn connect_search(
    ...
    search.connect_search_changed(move |entry| {
        if widgets.suppress_model_refresh.get() {
            return;
        }
        let query = entry.text().to_string();
        let revision = widgets.search_revision.get().wrapping_add(1).max(1);
        widgets.search_revision.set(revision);
        ...
        glib::timeout_add_local_once(Duration::from_millis(SEARCH_DEBOUNCE_MS), move || {
            if widgets.search_revision.get() != revision
                || widgets.search.text().as_str() != query.as_str()
            {
                return;
            }

            if is_search_query_active(query.as_str()) {
                refresh_search_data(&widgets, &data, query.as_str());
            } else {
                refresh_all_pages(&widgets, &data, "");
            }
            apply_search_mode(&widgets, &pool, &runtime, &data, query.as_str());
        });
    });
}

fn connect_window_search_typing(window: &adw::ApplicationWindow, widgets: &AppWidgets) {
    ...
    key_controller.connect_key_pressed(move |_, key, _, state| {
        if !controller_window.is_active() {
            return glib::Propagation::Proceed;
        }

        let has_control = state.contains(gtk::gdk::ModifierType::CONTROL_MASK);
        if has_control && (key == gtk::gdk::Key::k || key == gtk::gdk::Key::K) {
            search.grab_focus();
        ...
        if key == gtk::gdk::Key::Escape {
            let had_text = !search.text().is_empty();
            let had_focus = search.has_focus();
            if had_text {
                search.set_text("");
            }
            if had_focus {
                gtk::prelude::GtkWindowExt::set_focus(&controller_window, gtk::Widget::NONE);
            }
            return if had_text || had_focus {
        ...
        if focused_widget_accepts_text(&controller_window)
            || has_control
            || has_shortcut_modifier(state)
        {
            return glib::Propagation::Proceed;
        }
        ...
        insert_character_in_search(&search, character);
        glib::Propagation::Stop
    });
...
fn focused_widget_accepts_text(window: &adw::ApplicationWindow) -> bool {
    gtk::prelude::GtkWindowExt::focus(window)
        .is_some_and(|focus| focus.is::<gtk::Editable>() || focus.is::<gtk::TextView>())
}

fn has_shortcut_modifier(state: gtk::gdk::ModifierType) -> bool {
    state.intersects(
...
fn insert_character_in_search(search: &gtk::SearchEntry, character: char) {
    search.grab_focus();

    let mut position = if let Some((start, _)) = search.selection_bounds() {
        search.delete_selection();
        start
    } else {
...
fn connect_filter_controls(widgets: &AppWidgets, data: LoadedData) {
    ...
    control.connect_selected_notify(move |_| {
        if refresh_widgets.suppress_model_refresh.get() {
            return;
        }
...
fn open_target_from_search_result(
    widgets: &AppWidgets,
    pool: Rc<sqlx::SqlitePool>,
    runtime: Rc<tokio::runtime::Runtime>,
    data: LoadedData,
    ...
    open_target_in_current_tab(widgets, pool, runtime, data, target);
}

fn connect_search_results_activation(
    widgets: &AppWidgets,
    pool: Rc<sqlx::SqlitePool>,
    runtime: Rc<tokio::runtime::Runtime>,
    data: LoadedData,
) {
    let surface = widgets.search_results.sections.clone();
    let click = gtk::GestureClick::new();
    click.set_button(0);
    let click_widgets = widgets.clone();
    click.connect_pressed(move |gesture, _, x, y| {
        ...
        let Some(target) = target_at_position(surface.as_ref(), x, y) else {
            return;
        };

        if button == 2 {
            if !click_widgets.search.text().is_empty() {
                click_widgets.search_mode_active.set(false);
                click_widgets.search.set_text("");
            }
            open_target_in_new_tab(
                &click_widgets,
                pool.clone(),
            ...
            open_target_from_search_result(
                &click_widgets,
                pool.clone(),
                runtime.clone(),
                data.clone(),
    ...
    widgets.search_results.sections.add_controller(click);
}
...
fn load_move_detail_page(
    ...
    let revision = widgets.detail_loader.next_revision();
    widgets.move_detail.id.set_text(&format!("#{move_id}"));
    widgets.move_detail.title.set_text("Loading...");
    widgets.stack.set_visible_child_name("move-detail");

    let pool = pool.clone();
    let sender = widgets.detail_loader.sender.clone();
    runtime.spawn(async move {
        ...
        let _ = sender.send(DetailLoadResult::Move { revision, payload });
    });
}
...
fn load_ability_detail_page(
    ...
    let revision = widgets.detail_loader.next_revision();
    widgets.ability_detail.id.set_text(&format!("#{ability_id}"));
    widgets.ability_detail.title.set_text("Loading...");
    widgets.stack.set_visible_child_name("ability-detail");
    ...
    runtime.spawn(async move {
        ...
        .await
        .map_err(|error| error.to_string());
        let _ = sender.send(DetailLoadResult::Ability { revision, payload });
    });
}
...
fn load_item_detail_page(
    ...
    let revision = widgets.detail_loader.next_revision();
    widgets.item_detail.id.set_text(&format!("#{item_id}"));
    widgets.item_detail.title.set_text("Loading...");
    widgets.stack.set_visible_child_name("item-detail");
    ...
    runtime.spawn(async move {
        let payload = async {
            ...
            let locations =
                if let (Some(game), Some(item)) = (selected_game.as_ref(), detail.as_ref()) {
                    native::load_game_item_locations(&pool, &game.id, &item.name_key).await?
                } else {
                    Vec::new()
                };
            Ok::<_, sqlx::Error>(ItemDetailPayload {
                detail,
                locations,
                selected_game,
            })
        ...
        .await
        .map_err(|error| error.to_string());
        let _ = sender.send(DetailLoadResult::Item { revision, payload });
    });
}
...
fn load_detail(
    ...
    let selected_game = data.selected_game.clone();
    let revision = widgets.detail_loader.next_revision();
    widgets.detail.id.set_text(&format!("#{pokemon_id:03}"));
    widgets.detail.title.set_text("Loading...");
    if show {
        widgets.stack.set_visible_child_name("detail");
    }
    ...
    runtime.spawn(async move {
        let payload = async {
            ...
            let (game_abilities, game_moves, game_locations) = if let Some(game) = &selected_game {
                let Some(detail) = detail.as_ref() else {
                    return Ok::<_, sqlx::Error>(PokemonDetailPayload {
                        detail: None,
                        abilities: Vec::new(),
                        moves: Vec::new(),
                        evolution: None,
            ...
            Ok::<_, sqlx::Error>(PokemonDetailPayload {
                detail,
                abilities,
                moves,
                evolution,
        ...
        .await
        .map_err(|error| error.to_string());
        let _ = sender.send(DetailLoadResult::Pokemon { revision, payload });
    });
}
...
fn folded_search_tokens(query: &str) -> Vec<String> {
    native::fold_search_text(query)
        .split_whitespace()
        .map(ToOwned::to_owned)
        .collect()
}

fn lowercase_search_tokens(query: &str) -> Vec<String> {
    query
        .trim()
        .to_lowercase()
        .split_whitespace()
...
fn pokemon_matches_search_tokens(pokemon: &PokemonSummary, tokens: &[String]) -> bool {
    if tokens.is_empty() {
        return true;
    }
    ...
    let haystack = native::fold_search_text(&blob);
    tokens.iter().all(|token| haystack.contains(token))
}

fn text_matches_tokens(tokens: &[String], fields: &[&str]) -> bool {
    if tokens.is_empty() {
        return true;
    }
