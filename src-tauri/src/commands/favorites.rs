...
#[tauri::command]
pub async fn toggle_favorite(pokemon_id: i64, state: State<'_, AppState>) -> Result<bool, String> {
    let pool = &state.pool;

    // Check if already favorited
    let exists: Option<i64> =
        sqlx::query_scalar("SELECT pokemon_id FROM favorites WHERE pokemon_id = ?1")
            .bind(pokemon_id)
            .fetch_optional(pool)
            .await
